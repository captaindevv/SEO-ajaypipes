...
1. **Check `references/used-keywords.md` before accepting any seed keyword in `plan`.** This skill builds an entire hub-and-spoke content ecosystem from one seed — running it against a seed keyword that overlaps one of the 3 documented cannibalization clusters ("best/top pipe company/brand," "best pipe for plumbing," "which pipe is best" — ~25 of 102 posts already) would compound the site's worst existing problem at scale instead of fixing it. If the seed overlaps, stop and say so before generating a plan. Each per-post `blog-write` call in `execute` already inherits the full Ajay context automatically (see `skills/blog-write/SKILL.md`), so this check at the seed-selection stage is the one gap not otherwise covered.
2. **`references/linking.md` overrides this skill's generic link-density defaults for every post it writes.** `references/cluster-architecture.md`'s own link-count guidance (pillar-to-spoke, 2-3 cross-spoke links, etc.) still governs *cluster-internal* linking structure — but the Ajay-specific word-count tiers (10 links under 1,000 words, 13-15 under 1,500, 15-20 above) are the actual per-post floor, and cluster-internal links alone won't hit that floor on shorter spokes. Step 4 (Internal link matrix) and Step 3 of Execute Phase should pull the *additional* internal links needed to reach the tier from `references/internal-links.md` (the full 102-post + product-page inventory, not just sibling posts in this cluster) rather than stopping once the cluster's own pillar/spoke/cross-spoke links are placed.
3. **External links in cluster posts follow the same two-bucket rule as any other Ajay post** — `references/external-links.md`'s verified press mentions and government/standards sources only, never a generic web source, even though this skill's SERP-research phase surfaces plenty of other domains. The cluster context block passed to each `blog-write` call should carry this restriction explicitly.
4. **YouTube embeds, if any spoke/pillar includes one, are restricted to Ajay's own channel** per `references/youtube-links.md` — the cluster context block should not leave video sourcing to `blog-write`'s generic defaults.
