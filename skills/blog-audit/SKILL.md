...
1. **ajaypipes.com is WordPress — Step 1's Glob-based file discovery will find nothing.** The 102 published posts live on the CMS, not in a local `content/`/`posts/`/`blog/` directory. Use `references/used-keywords.md` as the post inventory (all 102 titles, primary keywords, publish dates, URLs) and fetch live URLs via WebFetch for content analysis instead of Glob/Read on local files.
2. **Step 3's cannibalization detection is already partly done** — `references/used-keywords.md` documents 3 clusters covering ~25 posts ("best/top pipe company/brand," "best pipe for plumbing," "which pipe is best"). Start from that list, then look for anything new outside it.
3. **Schema Validation Agent**: don't flag Organization/Article/Person/BreadcrumbList as missing — Yoast generates these sitewide (confirmed). The real, sitewide finding is FAQPage schema absent everywhere. Report it once as a systemic site issue, not per-post.
4. **Freshness Check Agent**: cross-reference `on-page-seo-ajaypipes.md` — the site's sitemap.xml is frozen at 2024-06-03 and excludes all 2025/2026 content. If freshness/indexation issues show up across many posts, the root cause is very likely this stale sitemap, not per-post content staleness. Say so explicitly rather than recommending individual content refreshes as the fix.
5. **Link Health Agent**: the known, confirmed structural problem on this site is that high-traffic informational posts link to zero product pages (see `on-page-seo-ajaypipes.md`). Report this as the top-priority link-health finding, not just generic orphan-page detection. Also check per-post link *density* against `references/linking.md`'s word-count tiers (10 links under 1,000 words, 13-15 under 1,500, 15-20 above) and check that any external links used are limited to the two approved buckets in `references/external-links.md` — flag any post linking externally outside those buckets, or falling short of the density tier, as a link-health finding alongside the zero-product-link check.
