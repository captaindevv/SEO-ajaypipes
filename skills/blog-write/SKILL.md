...
## Ajay Pipes Context — read these before doing anything

This deployment writes for ajaypipes.com. Read every file below before Phase 1. These are not optional background — they override the engine's generic defaults wherever the two conflict (e.g. this brand's humour rules are dry-to-warm, not the engine's default tone; this brand's CTA and internal-linking rules are stricter than the generic checklist).

1. `CLAUDE.md` — project rules, the business, the one non-negotiable rule (never write like the underdog)
2. `BRAND.md` — audience, positioning, editorial do/don't list, taboo phrases, topic scope
3. `VOICE.md` — pronoun stance, sentence/paragraph ceilings, headline patterns, readability target
4. `on-page-seo-ajaypipes.md` — **verified live-site findings, not assumptions**: FAQPage + Product schema are the real gaps (Organization/Article/Breadcrumb/Person already exist via Yoast, don't claim otherwise); stale sitemap; live unresolved cannibalization; product pages have no real CTA
5. `references/voice.md` — full sentence rhythm, banned words, "tells it's AI-written" checklist (Ajay-specific, supplements the generic list in Phase 6 below)
6. `references/humour.md` — the personality dial for this brand: dry-professional on product/technical pages, plain-spoken warmth (never jokes/puns) on homeowner-facing posts
7. `references/stats.md` — the only source for any Ajay number. Never round, never invent, never use an SEO/competitive metric (Authority Score, traffic, rank) in the article itself
8. `references/linking.md` — link-density target by word count (10 links under 1,000 words, 13-15 under 1,500, 15-20 above), sourced from `references/internal-links.md` (internal — product pages + 102 posts, tagged by topic) and `references/external-links.md` (external — verified press mentions + government/standards sources, the only two approved external domains). Video embeds pull from `references/youtube-links.md` only — never a third-party video, per `BRAND.md`'s no-competitor-comparison rule.
9. `references/stories.md` — real anecdotes only, one per post max, never invented. If empty or nothing topical fits, write the post without a story rather than inventing one
10. `references/opinions.md` — real technical stances only, one per post max, backed by a `stats.md` number. Same rule: skip rather than invent
11. `references/used-keywords.md` — **check before picking any primary keyword.** Three semantic-cannibalization clusters already exist covering ~25 of the 102 published posts ("best/top pipe company/brand," "best pipe for plumbing," "which pipe is best" — full lists in the file). Do not propose a topic that adds to one of these clusters; if the requested topic overlaps, say so and propose consolidating the existing posts instead of writing a new one.
...
**Author field for Ajay content**: this brand's voice is the functional "Ajay Technical Advisor" (see `VOICE.md`), not an invented named character. For the byline/Person schema, use a real named technical staff member if one has been supplied; otherwise default to an organizational byline ("Ajay Pipes Technical Team") rather than fabricating an individual's name and credentials. Flag to the user that a real named author with real credentials would strengthen E-E-A-T (the live-site audit found generic "ajaypipes" bylines with no real author signal — see `on-page-seo-ajaypipes.md`).
...
**Ajay-specific requirement**: at least one internal-link zone MUST point to a specific Ajay product page (Flowline Plus, Greenline, Drainline, Terraline, Agriline, or Deepline — whichever is topically relevant), not just to other blog posts. This is the single highest-impact structural fix identified on this site — the current top-traffic posts link to zero product pages (see `on-page-seo-ajaypipes.md`). A post that satisfies the generic "5-10 internal links" rule but links only to other blog posts has not satisfied this brand's requirement.
...
**Ajay-specific requirement**: `FAQPage` JSON-LD is a confirmed real gap on this site (verified directly — Organization/Article/Breadcrumb/Person schema already exist via Yoast, but zero pages have FAQ schema despite several having visible FAQ blocks; see `on-page-seo-ajaypipes.md`). Generate it via `/blog schema` for every post with an FAQ section — don't skip this step assuming Yoast already handles it.
...
#### Ajay-Specific Gates (in addition to the above — re-read `BRAND.md`, `VOICE.md`, `references/voice.md`, `references/humour.md` before checking these)
20. **No underdog framing** - Zero competitor names anywhere. Zero defensive phrasing ("even though," "unlike bigger brands," "catching up to"). Every authority claim traces to a real fact in `references/stats.md`.
21. **Taboo phrases scan** - Check against `BRAND.md`'s taboo list (unlock, leverage, seamless, world-class, best-in-class, comprehensive solution, "in today's fast-paced world," "we pride ourselves on," "contact us" as sole CTA, affordable/competitive pricing/quality service with no number attached).
22. **Humour register correct** - Product/technical/commercial pages: fully dry, zero personality flourishes. Homeowner-facing educational posts: at most one plain-spoken observation per 800-1,000 words, never a pun or joke. See `references/humour.md`.
23. **Product-page internal link present** - At least one internal link goes to a specific Ajay product page, not just other blog posts (see Phase 5f note above).
24. **Numbers traced to stats.md** - Every Ajay-specific number (years in business, certifications, test parameters, product specs) matches `references/stats.md` exactly. No rounding, no invented figures.
25. **Story/opinion sourced, not invented** - If a story or opinion appears, it's pulled from `references/stories.md` / `references/opinions.md` verbatim-adapted, never fabricated. If those files don't yet have topical material, the post proceeds without one rather than inventing.
26. **Keyword checked against used-keywords.md** - Primary keyword doesn't duplicate or semantically overlap an existing cluster (see the Ajay Pipes Context section above).
